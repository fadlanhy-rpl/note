{{-- resources/views/partials/sidebar-labels-list-items.blade.php --}}
@if(isset($labelsForSidebar) && $labelsForSidebar->isNotEmpty())
    @foreach($labelsForSidebar as $labelObj)
        <a href="{{ route('labels.show', $labelObj->id) }}"
            class="sidebar-item {{ (request()->routeIs('labels.show') && ($currentLabelParam = request()->route('label')) && $currentLabelParam instanceof \App\Models\Label && $currentLabelParam->id == $labelObj->id) ? 'active' : '' }} flex items-center">
